...
const bodySchema = z.object({
  ...
  type: z.enum(['all', 'title', 'description', 'keywords']).default('all'),
})
...
export async function POST(req: Request) {
  ...
  if (profile.plan !== 'pro') {
    return NextResponse.json(
      { error: '셀링 페이지 카피 생성은 Pro 플랜 전용 기능입니다.' },
      { status: 403 },
    )
  }
...
위 정보를 기반으로 플랫폼에 최적화된 카피를 JSON 형식으로 생성하세요.`
  ...
  try {
    ...
    return NextResponse.json({
      data: {
        titles: Array.isArray(parsed.titles) ? parsed.titles.slice(0, 3) : undefined,
